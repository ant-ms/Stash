...
import { WebSocketController, type WebSocketEventHandler } from './websocket-controller.svelte.js';
...
export class FCastController {
	 ...
	 */
	constructor(ip: string, port: number, deviceInfo?: Partial<InitialSenderMessage>) {
		this.deviceInfo = {
		...
		this.webSocketController = new WebSocketController(ip, port, eventHandlers);
	}
	...
	public play = (url: string, type: string): void => {
		this.webSocketController?.sendPacket(OPCODE.Play, new PlayMessage(type, url));
	};
	...
	public setVolume = (volume: number): void => {
		this.webSocketController?.sendPacket(OPCODE.SetVolume, new SetVolumeMessage(volume));
	};
	...
	public setSpeed = (speed: number): void => {
		this.webSocketController?.sendPacket(OPCODE.SetSpeed, new SetSpeedMessage(speed));
	};
	...
	private onWebSocketOpen = (): void => {
		...
		this.webSocketController?.sendPacket(OPCODE.Version, new VersionMessage(this.protocolVersion));
	};
	...
	private onPacketReceived = (opcode: number, body: object | null): void => {
		switch (opcode) {
			...
			case OPCODE.PlaybackError:
				this.onPlaybackError((body as PlaybackErrorMessage)?.message || 'Unknown playback error');
				break;
